Your human + AI workforce where humans and agents ship together. Assign to anyone — human or AI.
# **Hackathon II**
...
# **The Real Innovation: Human-Agent Task Management Platform**

> **One-liner:** Asana for humans and AI agents — assign tasks to either, track everything, approve what matters.

## **AI-First Architecture**

This is NOT an "app with AI features." This is an **AI-First platform** where:

| Traditional App | AI-First Platform |
| :---- | :---- |
| Humans use UI to manage tasks | **Agents connect via MCP** to work on tasks |
...
| AI helps you work | AI **does** the work |
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│   HUMANS (Web UI)                 AI AGENTS (MCP Server)     │
│   ───────────────                 ──────────────────────     │
...
## **What Humans Do (Web UI)**

| Area | Human Actions |
| :---- | :---- |
| **Projects** | Create project, add description |
| **Tasks** | Create task, set priority, set due date |
| **Assignment** | Assign task to human OR connected agent |
| **Review** | View agent work, approve or reject |
| **Audit** | See who did what, when |

## **What Agents Do (MCP Server)**

The platform exposes an MCP server. Agents connect to it and can:
...
**Hackathon Scope:** For the hackathon, agents are pre-registered (e.g., `@writing-agent`, `@research-agent`). Humans see them in a dropdown when assigning tasks. The MCP server authenticates agents by API key.
...
**Simple yet powerful:** The platform is a task database with two doors — a UI for humans and an MCP server for agents. Both work on tasks. Everything is logged automatically.

## **Why Conversational Todo is NOT the Innovation**

| Approach | Reality |
| :---- | :---- |
| "Add task buy milk" via chat | **Slower** than tapping a button |
| AI chatbot for CRUD | Over-engineered for simple operations |
| Voice commands for todo | Cool demo, nobody uses daily |

**The input method isn't the problem. WHO DOES THE WORK is the problem.**

A todo app where YOU still do all the work is just a prettier task list. The world has thousands of those.

## **The Paradigm Shift: AI Agents as Workers**

| Traditional Todo | Task Orchestration Platform |
| :---- | :---- |
| List of things **you** do | List of things **getting done** |
| You're the worker | You're the orchestrator |
| Manual progress updates | Agents report progress |
| Static tasks | Tasks spawn sub-tasks dynamically |
| Human executes | Human **reviews and approves** |

```
Traditional:
Human creates → Human does → Human completes
...
┌─────────────────────────────────────┐
│  Some tasks → HUMANS                │
│  Some tasks → FOREGROUND AGENTS     │
│  Some tasks → BACKGROUND AGENTS     │
└─────────────────────────────────────┘
      ↓
Coordinated progress tracking
      ↓
Human reviews/approves outcomes
```
...
| Feature | Description |
| :---- | :---- |
| **Unified Assignment** | Assign any task to a human OR an AI agent from the same interface |
| **Auto Audit Trail** | Automatic logging of who (human or agent) did what, when, and how |
| **Invite & Assign** | Invite team members (humans) and connect agents to your workspace |
| **Progress Tracking** | Real-time status from both human and agent workers |
| **Approval Gates** | Human review points before agent work is finalized |
| **Agent Connectivity** | Background agents report progress via MCP/webhooks |
...
│                                                         │
│  ┌─────────────────────────────────────────────────┐   │
│  │ 📝 Write investor report                        │   │
│  │ Assigned to: @writing-agent (AI)                │   │
│  │ Status: 🔄 In Progress (68%)                    │   │
│  │ Audit: Started 10:32 AM by system               │   │
│  └─────────────────────────────────────────────────┘   │
│                                                         │
│  ┌─────────────────────────────────────────────────┐   │
...
│  │ Status: ✅ Completed                            │   │
│  │ Audit: Completed 9:45 AM by sarah@company.com   │   │
│  └─────────────────────────────────────────────────┘   │
│                                                         │
│  ┌─────────────────────────────────────────────────┐   │
│  │ ⏳ Generate charts                              │   │
│  │ Assigned to: @viz-agent (AI)                    │   │
│  │ Status: ⏳ Waiting for approval                 │   │
│  │ Audit: Awaiting human review since 10:15 AM     │   │
│  └─────────────────────────────────────────────────┘   │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
**User says**: "Prepare the Q1 investor report"

**Traditional chatbot**: Creates 1 task. User does everything.

**Task Orchestration Platform**:

| Task | Assigned To | Status |
| :---- | :---- | :---- |
| Pull revenue data from DB | `data-agent` (background) | ✅ Done |
| Generate charts | `viz-agent` (background) | ✅ Done |
| Write executive summary | `writing-agent` (background) | 🔄 In progress |
| Review financials for accuracy | **Human (CFO)** | ⏳ Waiting |
| Final approval | **Human (CEO)** | ⏳ Blocked |

**The human didn't write the report. The human ORCHESTRATED and REVIEWED.**

## **SpecKit Integration (Future Vision)**

This hackathon's todo app is a stepping stone. The architecture you build can evolve into:

```
SpecKit generates development tasks
        ↓
Tasks go to orchestration platform
        ↓
Background agents execute (research, code, test)
        ↓
Humans review milestones
        ↓
Agents deploy
```

Example workflow:
- `spec-agent` writes feature specification (background)
- Human architect reviews spec
- `code-agent` implements backend (background)
- `code-agent` implements frontend (background)
- `test-agent` writes and runs tests (background)
...
| :---- | :---- | :---- |
| Delegate routine work to agents | Coordinate human + AI workers | Audit who (human or AI) did what |
| Focus on decisions, not execution | Async collaboration at scale | Compliance + accountability |
| Personal AI workforce | Shared agent pools | Multi-tenant, scoped permissions |

## **Your Challenge**

The minimum requirements build a todo app. The **real opportunity** is to build the **Human-Agent Task Management Platform** — the first task manager where humans and AI agents are equal citizens.

**Think of it as:**
- Asana, but AI agents can be assigned work
- Trello, but cards complete themselves
- Jira, but tickets get done by bots

**Don't just build a chatbot that creates tasks. Build infrastructure where tasks get DONE by a mix of humans and AI agents — with full audit trail of who did what.**
...
| :---- | :---: |
| **Human-Agent Task Management Platform** – Build the full vision: unified task assignment to humans OR AI agents, automatic audit trail (who did what, when), invite/assign team members, and approval gates for agent work | \+300 |
...
Better Auth can be configured to issue **JWT (JSON Web Token)** tokens when users log in. These tokens are signed with **asymmetric keys (RS256)** and can be verified by any service using the **public keys** from the JWKS endpoint — no shared secrets required.

# **How It Works**

* User logs in on Frontend → Better Auth creates a session and issues a JWT token (signed with RS256)
* Frontend makes API call → Includes the JWT token in the Authorization: Bearer \<token\> header
* Backend receives request → Fetches public keys from SSO's JWKS endpoint, verifies JWT signature
...
| :---- | :---- |
| **Better Auth Config** | Enable JWT plugin with JWKS (RS256) for asymmetric signing |
| **Frontend API Client** | Attach JWT token to every API request header |
| **FastAPI Backend** | Add middleware to verify JWT using JWKS public keys |
| **API Routes** | Filter all queries by the authenticated user's ID |

## **FastAPI Token Verification Example (Python)**

```python
from jwt import decode, PyJWKClient, InvalidTokenError
from fastapi import HTTPException, Depends, Request

# Configuration - only needs the JWKS URL, NO secrets!
JWKS_URL = "https://your-sso-server/api/auth/jwks"
ISSUER = "https://your-sso-server"
...
def verify_token(request: Request):
    ...
    try:
        # Fetch the correct public key based on token's 'kid' header
        ...
        payload = decode(
            token,
            signing_key,
            algorithms=["RS256"],  # Asymmetric - NOT HS256!
            issuer=ISSUER,
...
async def get_tasks(user_id: str, claims=Depends(verify_token)):
    # Verify the token's user matches the URL
...
**Key Points:**
- Backend does NOT need `BETTER_AUTH_SECRET`
- Backend does NOT need to be registered as an OAuth client
- `PyJWKClient` handles key caching and rotation automatically
- Uses RS256 (asymmetric) — attacker with public key cannot forge tokens

# **JWKS-Based Verification (Industry Standard)**

Better Auth uses **asymmetric cryptography (RS256)** for token signing:

* **SSO Server** holds the **private key** (signs tokens) — never shared
* **FastAPI Backend** fetches **public keys** from the JWKS endpoint — no secrets needed

**JWKS Endpoint:** `https://your-sso-server/api/auth/jwks`

This is the same approach used by Auth0, Cognito, Okta, and Google. The backend does NOT need to be registered as a client — it simply fetches public keys to verify tokens.

# **Security Benefits**

| Benefit | Description |
| :---- | :---- |
| **No Shared Secrets** | Backend only needs public keys — if compromised, attacker cannot forge tokens |
| **User Isolation** | Each user only sees their own tasks |
| **Stateless Auth** | Backend verifies tokens locally using cached JWKS (no SSO calls per request) |
| **Token Expiry** | Short-lived access tokens (5-15 min recommended) limit exposure |
| **Key Rotation** | JWKS supports automatic key rotation without service restarts |

**⚠️ Important:** Do NOT use shared secrets (HS256) for distributed systems. If the secret leaks from any service, attackers can forge tokens for ALL services.
