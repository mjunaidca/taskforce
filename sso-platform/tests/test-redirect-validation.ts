...
test("Get Trusted Origins - Contains AI Native", () => {
  const origins = getTrustedOrigins();
  return origins.includes("https://ai-native.taskflow.org");
});
...
test("Valid Redirect - AI Native Origin", () => {
  return isValidRedirectUrl("https://ai-native.taskflow.org");
});

// Test 9: AI Native with path is valid
test("Valid Redirect - AI Native with Path", () => {
  return isValidRedirectUrl("https://ai-native.taskflow.org/dashboard");
});
...
// Test 15: Taskflow.org is valid
test("Valid Redirect - Taskflow.org", () => {
  const origins = getTrustedOrigins();
  // Check if taskflow.org is in trusted origins
  const hasTaskflow = origins.some(origin => origin.includes("taskflow.org"));
  return hasTaskflow && isValidRedirectUrl("https://taskflow.org/home");
});
...
test("Invalid Redirect - AI Native OAuth Callback", () => {
  return !isValidRedirectUrl("https://ai-native.taskflow.org/auth/callback");
});
...
test("Valid Redirect - Deep Path Without Callback", () => {
  return isValidRedirectUrl("https://ai-native.taskflow.org/projects/123/settings");
});
