...
Taskflow uses a **hybrid multi-tenant model**:
- **Default Organization**: All users automatically join "Taskflow" on signup
...
✅ Creates default "Taskflow" organization
✅ Adds admin as owner
...
3. User gets JWT token:
   {
     tenant_id: "taskflow-default-org-id",
     organization_ids: ["taskflow-default-org-id"],
     org_role: "member"
   }
```
...
3. User now member of 2 orgs:
   {
     tenant_id: "taskflow-default-org-id",  // Still active org
     organization_ids: [
       "taskflow-default-org-id",
       "mit-school-id"
     ],
     org_role: "member"
   }
...
2. Verify default org exists:
   ```sql
   SELECT * FROM organization WHERE id = 'taskflow-default-org-id';
   ```
3. Manually add user to org:
   ```sql
   INSERT INTO member (id, user_id, organization_id, role, created_at)
   VALUES (uuid(), 'user-id', 'taskflow-default-org-id', 'member', NOW());
   ```
...
```typescript
{
  tenant_id: "taskflow-default-org-id",  // ← Always present!
  organization_ids: ["taskflow-default-org-id"],
  org_role: "member"
}
```
