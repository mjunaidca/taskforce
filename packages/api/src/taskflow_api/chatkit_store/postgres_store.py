...
class PostgresStore(Store[RequestContext]):
    ...
    def __init__(
        ...
        logger.info("PostgresStore initialized for TaskFlow ChatKit")
    ...
    def _create_engine(self, config: StoreConfig) -> AsyncEngine:
        ...
        return create_async_engine(
            ...
            connect_args={
                "command_timeout": 30,
                "server_settings": {
                    "application_name": "taskflow_chatkit",
                    "jit": "off",
    ...
    async def load_threads(
        ...
        async with self.session_factory() as session:
            ...
            threads = [ThreadData.model_validate(row[0]).thread for row in result.fetchall()]
